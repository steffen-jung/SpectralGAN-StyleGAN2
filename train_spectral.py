...
from training import training_loop_spectral
...
def setup_training_options(
    ...
    args.loss_args = dnnlib.EasyDict(func_name='training.loss_spectral.stylegan2')
...
def run_training(outdir, seed, dry_run, **hyperparam_options):
    ...
    with dnnlib.util.Logger(os.path.join(training_options.run_dir, 'log.txt')):
        training_loop_spectral.training_loop(**training_options)
...
def main():
    ...
    # group.add_argument('--i', help='Iteration', type=int, default=0, metavar='INT')
