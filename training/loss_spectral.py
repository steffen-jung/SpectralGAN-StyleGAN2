...
def report_loss(aug, G_loss, D_loss, D_spectral_loss, G_reg=None, D_reg=None, D_spectral_reg=None):
    ...
    terms.D_spectral_loss = report_stat(aug, 'Loss/D_spectral/loss', D_spectral_loss)
    ...
    if D_spectral_reg is not None: terms.D_spectral_reg = report_stat(aug, 'Loss/D_spectral/reg', D_spectral_reg)
    return terms
...
def stylegan2(G, D, D_spectral, aug, fake_labels, real_images, real_labels, r1_gamma=10, pl_minibatch_shrink=2, pl_decay=0.01, pl_weight=2, **_kwargs):
    ...
    real_images_spectral = (real_images + 1) / 2
    fake_images_spectral = (G_fake.images + 1) / 2
    D_spectral_fake = eval_D(D_spectral, aug, fake_images_spectral, fake_labels, report='fake_spectral')
    D_spectral_real = eval_D(D_spectral, aug, real_images_spectral, real_labels, report='real_spectral')

    # Non-saturating logistic loss from "Generative Adversarial Nets".
    with tf.name_scope('Loss_main'):
        G_loss = tf.nn.softplus(-D_fake.scores) + tf.nn.softplus(-D_spectral_fake.scores) # -log(sigmoid(D_fake.scores)), pylint: disable=invalid-unary-operand-type
        G_loss = G_loss * 0.5
        ...
        D_spectral_loss = tf.nn.softplus(D_spectral_fake.scores)
        D_spectral_loss += tf.nn.softplus(-D_spectral_real.scores)
        G_reg = 0
        D_reg = 0
        D_spectral_reg = 0
    ...
    if r1_gamma != 0:
        ...
        with tf.name_scope('Loss_R1_spectral'):
            r1_grads = tf.gradients(tf.reduce_sum(D_spectral_real.scores), [real_images])[0]
            r1_penalty = tf.reduce_sum(tf.square(r1_grads), axis=[1,2,3])
            r1_penalty = report_stat(aug, 'Loss/r1_penalty_spectral', r1_penalty)
            D_spectral_reg += r1_penalty * (r1_gamma * 0.5)
    ...
    return report_loss(aug, G_loss, D_loss, D_spectral_loss, G_reg, D_reg, D_spectral_reg)
