...
from training._SpectralDiscriminatorTF import SpectralLoss
...
def D_spectral(
    ...
    **_kwargs,                          # Ignore unrecognized keyword args.
    ...
    spectral_loss = SpectralLoss(rows=1024, cols=1024)
    ...
    x = spectral_loss.spectral_vector(images_in)

    # Output layer (always trainable).
    with tf.variable_scope('Output_Spectral'):
        x = apply_bias_act(dense_layer(x, fmaps=score_max))
    scores_out = x[:, :score_size]
