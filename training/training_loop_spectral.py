...
def training_loop(
    ...
    with tf.device('/gpu:0'):
        ...
        D_spectral = tflib.Network(
            'D_spectral',
            num_channels = training_set.shape[0],
            resolution = training_set.shape[1],
            label_size = training_set.label_size,
            func_name = "training.networks_spectral.D_spectral",
        )
        Gs = G.clone('Gs')
        ...
        if resume_pkl is not None:
            ...
            with dnnlib.util.open_url(resume_pkl) as f:
                load_resume = pickle.load(f)
                
            if len(load_resume) == 4:
                rG, rD, rD_spectral, rGs = load_resume
                D_spectral.copy_vars_from(rD_spectral)
            else:
                rG, rD, rGs = load_resume
    ...
    D_spectral_opt_args = dict(D_opt_args)
    for args, reg_interval in [(G_opt_args, G_reg_interval), (D_opt_args, D_reg_interval), (D_spectral_opt_args, D_reg_interval)]:
    ...
    D_spectral_opt = tflib.Optimizer(name='TrainD_spectral', **D_opt_args)
    G_reg_opt = tflib.Optimizer(name='RegG', share=G_opt, **G_opt_args)
    D_reg_opt = tflib.Optimizer(name='RegD', share=D_opt, **D_opt_args)
    D_spectral_reg_opt = tflib.Optimizer(name='RegD_spectral', share=D_opt, **D_opt_args)

    print('Constructing training graph...')
    data_fetch_ops = []
    training_set.configure(minibatch_gpu)
    for gpu, (G_gpu, D_gpu, D_spectral_gpu) in enumerate(zip(G_gpus, D_gpus, D_spectral_gpus)):
        with tf.name_scope(f'Train_gpu{gpu}'), tf.device(f'/gpu:{gpu}'):
            ...
            terms = dnnlib.util.call_func_by_name(G=G_gpu, D=D_gpu, D_spectral=D_spectral_gpu, aug=aug, fake_labels=fake_labels, real_images=real_images_var, real_labels=real_labels_var, **loss_args)
            if lazy_regularization:
                ...
                if terms.D_spectral_reg is not None: D_spectral_reg_opt.register_gradients(tf.reduce_mean(terms.D_spectral_reg * D_reg_interval), D_spectral_gpu.trainables)
            else:
                if terms.G_reg is not None: terms.G_loss += terms.G_reg
                if terms.D_reg is not None: terms.D_loss += terms.D_reg
                if terms.D_spectral_reg is not None: terms.D_spectral_loss += terms.D_spectral_reg
            ...
            D_spectral_opt.register_gradients(tf.reduce_mean(terms.D_spectral_loss), D_spectral_gpu.trainables)

    print('Finalizing training ops...')
    data_fetch_op = tf.group(*data_fetch_ops)
    G_train_op = G_opt.apply_updates()
    D_train_op = D_opt.apply_updates()
    D_spectral_train_op = D_spectral_opt.apply_updates()
    G_reg_op = G_reg_opt.apply_updates(allow_no_op=True)
    D_reg_op = D_reg_opt.apply_updates(allow_no_op=True)
    D_spectral_reg_op = D_spectral_reg_opt.apply_updates(allow_no_op=True)
    ...
    while not done:
        ...
        for _repeat_idx in range(minibatch_repeats):
            ...
            if len(rounds) == 1:
                ...
                if run_G_reg:
                    tflib.run(G_reg_op)
                tflib.run([D_train_op, D_spectral_train_op, Gs_update_op], {Gs_beta_in: Gs_beta})
                if run_D_reg:
                    tflib.run([D_reg_op, D_spectral_reg_op])
            ...
            # Slow path with gradient accumulation.
                ...
                for _round in rounds:
                    ...
                    tflib.run([D_train_op, D_spectral_train_op])
                    if run_D_reg:
                        tflib.run([D_reg_op, D_spectral_reg_op])
        ...
        if done or cur_tick < 0 or cur_nimg >= tick_start_nimg + kimg_per_tick * 1000:
            ...
            if network_snapshot_ticks is not None and (done or cur_tick % network_snapshot_ticks == 0):
                ...
                with open(pkl, 'wb') as f:
                    pickle.dump((G, D, D_spectral, Gs), f)
                if len(metrics):
